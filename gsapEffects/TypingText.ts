...
export const typingTextEffectMultiple: (
  cursor: HTMLSpanElement | null,
  box: HTMLSpanElement | null,
  hi: HTMLSpanElement | null,
  text: HTMLSpanElement | null
) => void = (cursor, box, hi, text) => {
  ...
  boxTl
    ...
    .to(box, {
      ...
      height: "7vw",
      ease: "elastic.out",
      onComplete: (callback: gsap.Callback) => {
        masterTl.play(), callback();
      },
    })
    .to(box, {
      duration: 0.5,
...
export const typingTextEffectSingle: (
  text: HTMLSpanElement | null,
  box: HTMLSpanElement | null,
  sentence: string
) => void = (text, box, sentence) => {
  let boxTl = gsap.timeline();
